...
export const eliminarInventario = async (req, res) => {
  ...
  try {
    ...
    const inventarioEliminado = await Inventario.destroy({ where: { id_inventario: id } });
...
export const editarInventario = async (req, res) => {
  ...
  const { id_producto, id_bodega, stock, ubicacion_en_bodega, fecha_entrada, fecha_caducidad } = req.body; // Obtener los datos actualizados del producto desde el cuerpo de la solicitud
  try {
    // Buscar el producto por su ID y actualizar sus datos
    const bodegaActualizado = await Inventario.update(
      { id_producto, id_bodega, stock, ubicacion_en_bodega, fecha_entrada, fecha_caducidad },
